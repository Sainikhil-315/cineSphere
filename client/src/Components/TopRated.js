...
function TopRated(props) {
    ...
    useEffect(() => {
        ...
        const fetchPopularMovies = async () => {
            const url = `https://api.themoviedb.org/3/movie/top_rated?api_key=${props.apiKey}&page=${page}`;
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.5 }} style={{ marginTop: "30px" }}>
            <h2 className='text-warning d-flex justify-content-center'>Top Rated Movies</h2>
